...
## Per-user module permissions (added 2026-09-18)

A Store User account can now be scoped to only the modules it needs (Customers, Vehicles, Items, Suppliers, Purchases, Job Cards, Invoices, Reports, Employees, Attendance, Salary), set as a checklist when creating the user or edited later from `/dashboard/users`. Admin remains a fixed superuser — it always has every module regardless of what's stored in its `permissions` field — and Users/Settings management stays Admin-only forever, not part of the grantable list (a permission-scoped account could otherwise create other logins or restore the database).

Enforced on the backend (a new `requirePermission()` middleware gates each module's router — matching but independent from `authorize("ADMIN")`, which still gates Users/Backup) and mirrored on the frontend (the sidebar hides modules a Store User can't open, and each page redirects if visited directly). Like `role`, `permissions` is embedded in the access token rather than looked up per request, so a change takes effect on the next token refresh (worst case ~15 minutes), not instantly. The dashboard's summary widgets and landing page stay visible to everyone regardless of permissions; only the deeper `/dashboard/reports` analytics require the Reports permission specifically.
